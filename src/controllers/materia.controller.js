...
//! FUNCION aysnc DELETE MATERIA (DESACTIVAR) como es desactivar y no borrar usamos una patch tamb podemos usar un put;
async function deleteMateria(req, res) {
  const { id } = req.params; // Obtener el ID desde los parámetros del params
  ...
  try {
    ...
    // Verificar si la materia ya está desactivada
    if (materia[0].activo === 0) {
      return res.status(400).json({ message: 'La materia ya está DESACTIVADA' });
    }

    // Desactivar la materia (establecer activo a false como 0 o 1 en db)
    await pool.query('UPDATE Materia SET activo = FALSE WHERE id_materia = ?', [id]);

    res.status(200).json({ message: 'Materia desactivada exitosamente' });
  } catch (error) {
...
//! FUNCION ASYNC REACTIVAR MATERIA (ACTIVAR) como es reactivar y no borrar usamos una put ademas en la anterior usamo un patch;
async function reactivarMateria(req, res) {
    ...
    try {
        ...
        // Verificar si la materia ya está desactivada
        if (materia[0].activo === 1) {
            return res.status(400).json({ message: 'La materia ya está ACTIVADA' });
        }
        // Reactivar la materia (establecer activo a true)
        await pool.query('UPDATE Materia SET activo = TRUE WHERE id_materia = ?', [id]);

        res.status(200).json({ message: 'Materia reactivada exitosamente' });
    } catch (error) {
